...
-- Task1 Вывести всех котиков по магазинам по id (условие соединения shops.id = cats.shops_id)
SELECT cats.name, shops.shopname FROM cats
LEFT JOIN shops
ON shops.id = cats.shops_id;

-- Task2 Вывести магазин, в котором продается кот “Мурзик” Var1
SELECT shops.shopname FROM cats
LEFT JOIN shops
ON shops.id = cats.shops_id
WHERE cats.name = "Murzik";

-- Task2 Вывести магазин, в котором продается кот “Мурзик” Var2
SELECT shops.shopname FROM cats, shops
WHERE shops.id = cats.shops_id AND name = "Murzik";

-- Task3 Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT cats.name, shops.shopname
FROM cats
LEFT JOIN shops
ON cats.shops_id = shops.id
WHERE cats.shops_id NOT IN (
...
SELECT analysis.an_name, orders.ord_datetime, analysis.an_price
FROM analysis
LEFT JOIN orders
ON analysis.an_id = orders.ord_an
WHERE orders.ord_datetime >= '2020-02-05' AND orders.ord_datetime < '2020-12-05'
