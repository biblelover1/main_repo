...
SELECT post, MAX(salary) FROM staff
GROUP BY post;

-- Найдите кол-во сотрудников сo специальностью (post) «Рабочий» в возрасте от 24 до 49 лет включительно.
SELECT COUNT(id) as `Count of workers` FROM staff
WHERE post = "Рабочий" AND age > 24 AND age <= 49;

-- Найдите количество специальностей
SELECT COUNT(DISTINCT post) as `Count of posts` FROM staff;
